...
import FlaggedConsultationsRowActions from "./flagged-consultations/FlaggedConsultationsRowActions";
import FlaggedConsultationsConfirmModal from "./flagged-consultations/FlaggedConsultationsConfirmModal";
...
export default function FlaggedConsultations() {
    ...
    return (
      ...
      `}</style>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {loading ? (
                                    ...
                                    filteredRows.map((row, index) => (
                                        <tr
                                            key={row.flagId || row.consultId || row.id}
                                            className={cls(
                                                "group",
                                        ...
                                        >
                                                    ...
                                                    <div className="min-w-0">
                                                        <div className="truncate text-[14px] cursor-pointer font-bold text-slate-900 transition-colors group-hover:text-sky-700"
                                                             title="View Consultant Details"
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    navigate(`/admin/consultants/profile/${row?.consultationId}`);
                                                                }}
                                                        >
                                                            {row.consultationName || row.consultName || "-"}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="border-b border-slate-100 px-6 py-4.5 text-right align-middle">
                                                <FlaggedConsultationsRowActions
                                                    onReview={() => handleReview(row)}
                                                    onSuspend={() => setModal({ type: "suspend", item: row, title: "Suspend Consultant" })}
                                                    onClear={() => setModal({ type: "clear", item: row, title: "Clear Flag" })}
                                                    onEscalate={() => setModal({ type: "escalate", item: row, title: "Escalate to Investigation" })}
                                                    onPenalty={() => setModal({ type: "penalty", item: row, title: "Apply Penalty" })}
                                                />
                                            </td>
                                        </tr>
                                    ))
                                        ...
                                        <td colSpan={9} className="px-6 py-32 text-center">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <div className="text-lg font-bold tracking-tight text-slate-900">
                                                    No flagged consultations found
                                                </div>
                            ...
                            <div className="shrink-0 border-b border-slate-200 bg-gradient-to-br from-slate-50 via-white to-sky-50/30 px-6 py-6">
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm font-medium text-slate-500">Refine flagged consultation records</p>
                                    </div>
            ...
            <FlaggedConsultationsConfirmModal
                modal={modal}
...
function ConsultantLogo({ src, alt }) {
    const [imgSrc, setImgSrc] = useState(src || "https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=600&auto=format&fit=crop&q=60");

    useEffect(() => {
        setImgSrc(src || "https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=600&auto=format&fit=crop&q=60");
    }, [src]);

    return (
        <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-[0_4px_12px_rgba(15,23,42,0.08)]">
            <img
                src={imgSrc}
                alt={alt}
                loading="lazy"
                onError={() => setImgSrc("https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=600&auto=format&fit=crop&q=60")}
