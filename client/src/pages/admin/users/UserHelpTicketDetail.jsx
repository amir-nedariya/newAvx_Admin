...
import { getUserHelpTicketById, resolveUserHelpTicket } from "../../../api/user.api";
...
const UserHelpTicketDetail = () => {
  ...
  const fetchTicketDetails = async () => {
    try {
      setLoading(true);
      const res = await getUserHelpTicketById(id);
      if (res.status === "OK") {
        setTicket(res.data);
      } else {
  ...
  const handleConfirmSolve = async () => {
    try {
      ...
      const res = await resolveUserHelpTicket(payload);
      if (res.status === "OK") {
        setTicket((prev) => ({
          ...prev,
          status: "SOLVED",
  ...
  if (!ticket) {
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center bg-slate-50/50 p-6 text-center">
        ...
        <button
          onClick={() => navigate("/admin/users", { state: { activeTab: "help-center" } })}
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white p-5 border border-slate-100 rounded-3xl shadow-[0_8px_30px_rgba(15,23,42,0.04)]">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-extrabold text-slate-900 leading-tight truncate" title={ticket.subject}>
              {ticket.subject}
            </h1>
            <p className="text-xs text-slate-400 mt-0.5">
              Submitted by{" "}
              <span className="font-semibold text-slate-600">
                {ticket.userFullName || ticket.consultantName || "User"}
              </span>
            </p>
            {(ticket.category || ticket.subCategory) && (
              <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                ...
                {ticket.category && (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-semibold">
                    {ticket.category}
                  </span>
                )}
                {ticket.category && ticket.subCategory && (
                ...
                {ticket.subCategory && (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-600 text-xs font-semibold">
                    {ticket.subCategory}
                  </span>
                )}
              </div>
        ...
        <div className="pt-6 border-t border-slate-100 space-y-3.5 text-xs">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 bg-white p-5 rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(15,23,42,0.02)] text-left">
            ...
            <div>
              ...
              <div className="mt-1">
                <span className={cls("inline-flex rounded-full border px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wide", statusBadge(ticket.ticketStatus || ticket.status))}>
          ...
          <div className="w-full max-w-[420px] bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <p className="text-[11px] text-slate-400 italic">
                ...
                <span className="text-emerald-600 font-semibold font-sans">SOLVED</span> and
                trigger the backend resolution workflow.
              </p>
            </div>
...
export default UserHelpTicketDetail;
